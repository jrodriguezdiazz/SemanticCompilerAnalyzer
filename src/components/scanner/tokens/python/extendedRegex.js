...
const decimalIntegerLiteral = new RegExp(
  "(0|(" + nonZeroDigit.source + decimalDigits.source + "?))"
);
const signedInteger = new RegExp("((\\+|-)?" + decimalDigits.source + ")");
const exponentIndicator = new RegExp("(e|E)");
const exponentPart = new RegExp(
  "(" + exponentIndicator.source + signedInteger.source + ")"
);
const decimalLiteral = new RegExp(
  "((" +
...
const numericLiteral = new RegExp(
  "(" + decimalLiteral.source + "|" + hexIntegerLiteral.source + ")"
);

// String Literal
const singleEscapeCharacter = new RegExp(/('|"|\\|b|f|n|r|t|v)/);
const escapeCharacter = new RegExp(
  "(" + singleEscapeCharacter.source + "|" + decimalDigit.source + "|x|u)"
);
const nonEscapeCharacter = new RegExp(
  "([^" +
    escapeCharacter.source.replace(/\[|\]/g, "") +
    "|" +
    lineTerminator.source +
    "])"
);
const characterEscapeSequence = new RegExp(
  "(" + singleEscapeCharacter.source + "|" + nonEscapeCharacter.source + ")"
);
const hexEscapeSequence = new RegExp(
  "(x" + hexDigit.source + hexDigit.source + ")"
);
...
const escapeSequence = new RegExp(
  "(" +
    characterEscapeSequence.source +
    "|(0(?!" +
    decimalDigit.source +
    "))" +
    "|" +
...
const lineContinuation = new RegExp(
  "(\\\\" + lineTerminatorSequence.source + ")"
);
const doubleStringCharacter = new RegExp(
  '(([^\\\\"' +
    lineTerminator.source +
    "])|(\\\\" +
...
const singleStringCharacter = new RegExp(
  "(([^\\\\'" +
    lineTerminator.source +
    "])|(\\\\" +
    escapeSequence.source +
...
const singleStringCharacters = new RegExp(
  "(" + singleStringCharacter.source + "+)"
);
const stringLiteral = new RegExp(
  '(("' +
    doubleStringCharacters.source +
    "?\")|('" +
...
// Regex Literals
const regularExpressionNonTerminator = new RegExp(
  "([^" + lineTerminator.source + "])"
);
...
const regularExpressionChars = new RegExp(
  "(|" + regularExpressionChar.source + "+)"
);
const regularExpressionBody = new RegExp(
  "(" + regularExpressionFirstChar.source + regularExpressionChars.source + ")"
);
const regularExpressionLiteral = new RegExp(
  "(/" +
    regularExpressionBody.source +
    "/" +
...
const fullRegularExpressionLiteral = new RegExp(
  regularExpressionLiteral.source
);
